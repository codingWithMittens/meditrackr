...
const MedicationTable = ({ medications, selectedFields, pharmacies, providers }) => {
  ...
  return (
    <div className="overflow-x-auto border border-gray-300 rounded-lg shadow-sm print:overflow-visible print:border-none print:shadow-none">
      <table className="min-w-full border-collapse print:table-auto">
      <thead>
        <tr className="bg-gray-100">
          {selectedFieldKeys.map(field => (
            <th key={field} className="border-r border-b border-gray-300 px-4 py-3 text-left font-semibold whitespace-nowrap min-w-0">
              {fieldHeaders[field]}
            </th>
          ))}
        </tr>
      ...
      <tbody>
        {medications.sort((a, b) => a.name.localeCompare(b.name)).map((med, index) => (
          <tr key={med.id} className={`${index % 2 === 0 ? 'bg-white' : 'bg-gray-50'} hover:bg-blue-50/50 transition-colors`}>
            {selectedFieldKeys.map((field, fieldIndex) => (
              <td key={field} className={`border-r border-b border-gray-300 px-4 py-3 align-top min-w-0 ${
                field === 'name' ? 'font-medium' : ''
              } ${
                ['providerDetails', 'pharmacyDetails', 'notes', 'symptoms'].includes(field) ? 'max-w-xs' : 'max-w-sm'
              }`}>
                <div className="truncate" title={typeof renderFieldContent(med, field) === 'string' ? renderFieldContent(med, field) : ''}>
